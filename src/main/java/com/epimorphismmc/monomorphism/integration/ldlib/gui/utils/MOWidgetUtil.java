package com.epimorphismmc.monomorphism.integration.ldlib.gui.utils;
...
public class MOWidgetUtil {
    public static IGuiTexture createTextOverlay(String translationKey) {
        return new IGuiTexture() {
            ...
            @OnlyIn(Dist.CLIENT)
            public void draw(
                    GuiGraphics graphics, int mouseX, int mouseY, float x, float y, int width, int height) {
                ...
                graphics.drawString(
                        fontRenderer,
                        s,
                        (int) ((x + (width / 3f)) * 2 - fontRenderer.width(s) + 23),
