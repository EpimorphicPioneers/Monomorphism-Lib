...
public class StructureUtil {
    public static final BlockPattern EMPTY_PATTERN =
            new BlockPattern(
                    new TraceabilityPredicate[0][0][0], new RelativeDirection[0], new int[0][0], new int[0]) {
                @Override
    ...
    public static BlockState getTierController(
            IMachineBlock controller, int tier, Direction direction) {
        return controller.getRotationState() == RotationState.NONE
                ...
                : controller
                        .self()
                        .defaultBlockState()
                        .setValue(controller.getRotationState().property, direction)
                        .setValue(MOBlockProperties.STRUCTURE_TIER, tier);
    }
    ...
    public static List<MultiblockShapeInfo> getMatchingShapes(
            MOBlockPattern blockPattern, int maxIndex) {
        int[][] aisleRepetitions = blockPattern.aisleRepetitions;
        var pages =
                repetitionCandidates(blockPattern, new ArrayList<>(), aisleRepetitions, new Stack<>());
    ...
    public static List<MultiblockShapeInfo> getTierMatchingShapes(
            MOBlockPattern blockPattern, int index) {
        int[][] aisleRepetitions = blockPattern.aisleRepetitions;
    ...
    private static List<MultiblockShapeInfo> repetitionDFS(
        ...
        if (repetitionStack.size() == aisleRepetitions.length) {
            ...
            for (int i = aisleRepetitions[repetitionStack.size()][0];
                    i <= aisleRepetitions[repetitionStack.size()][1];
                    i++) {
