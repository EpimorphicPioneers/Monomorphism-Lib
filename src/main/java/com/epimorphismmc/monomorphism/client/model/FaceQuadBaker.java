...
public class FaceQuadBaker {
    ...
    public BakedQuad bakeQuad(
        ...
        float f1 =
                (blockfaceuv.uvs[0] + blockfaceuv.uvs[0] + blockfaceuv.uvs[2] + blockfaceuv.uvs[2]) / 4.0F;
        float f2 =
                (blockfaceuv.uvs[1] + blockfaceuv.uvs[1] + blockfaceuv.uvs[3] + blockfaceuv.uvs[3]) / 4.0F;
        ...
        int[] aint = this.makeVertices(
                blockfaceuv,
                pSprite,
                pFacing,
                this.setupShape(pPosFrom, pPosTo),
    ...
    public static BlockFaceUV recomputeUVs(
            BlockFaceUV pUv, Direction pFacing, Transformation pModelRotation) {
        Matrix4f matrix4f = BlockMath.getUVLockTransform(
                        pModelRotation, pFacing, () -> "Unable to resolve UVLock for model")
                .getMatrix();
        ...
        float f12 = (float) Math.toRadians(pUv.rotation);
        Vector3f vector3f = new Vector3f(Mth.cos(f12), Mth.sin(f12), 0.0F);
        Matrix3f matrix3f = new Matrix3f(matrix4f);
        vector3f = matrix3f.transform(vector3f);
        int i = Math.floorMod(
                -((int) Math.round(Math.toDegrees(Math.atan2(vector3f.y(), vector3f.x())) / 90.0D)) * 90,
                360);
    ...
    private int[] makeVertices(
        ...
        for (int i = 0; i < 4; ++i) {
            this.bakeVertex(
                    aint, i, pOrientation, pUvs, pPosDiv16, pSprite, pRotation, pPartRotation, pShade);
        }
    ...
    private void bakeVertex(
        ...
        FaceInfo.VertexInfo faceinfo$vertexinfo =
                FaceInfo.fromFacing(pFacing).getVertexInfo(pVertexIndex);
        Vector3f vector3f = new Vector3f(
                pPosDiv16[faceinfo$vertexinfo.xFace],
                pPosDiv16[faceinfo$vertexinfo.yFace],
                pPosDiv16[faceinfo$vertexinfo.zFace]);
    ...
    private void fillVertex(
        ...
        pVertexData[i + 3] = -1;
        pVertexData[i + 4] =
                Float.floatToRawIntBits(pSprite.getU((double) pBlockFaceUV.getU(pVertexIndex) * .999
                        + pBlockFaceUV.getU((pVertexIndex + 2) % 4) * .001));
        pVertexData[i + 4 + 1] =
                Float.floatToRawIntBits(pSprite.getV((double) pBlockFaceUV.getV(pVertexIndex) * .999
                        + pBlockFaceUV.getV((pVertexIndex + 2) % 4) * .001));
    }
    ...
    private void applyElementRotation(Vector3f pVec, @Nullable BlockElementRotation pPartRotation) {
        if (pPartRotation != null) {
            ...
            var quaternion =
                    new Quaternionf().rotateAxis((float) Math.toRadians(pPartRotation.angle()), vector3f);
            ...
            this.rotateVertexBy(
                    pVec, new Vector3f(pPartRotation.origin()), new Matrix4f().rotate(quaternion), vector3f1);
        }
    }
    ...
    public void applyModelRotation(Vector3f pPos, Transformation pTransform) {
        if (pTransform != Transformation.identity()) {
            this.rotateVertexBy(
                    ...
                    pTransform.getMatrix(),
                    new Vector3f(1.0F, 1.0F, 1.0F));
        }
    }

    private void rotateVertexBy(
            Vector3f pPos, Vector3f pOrigin, Matrix4f pTransform, Vector3f pScale) {
        var vector4f =
                new Vector4f(pPos.x() - pOrigin.x(), pPos.y() - pOrigin.y(), pPos.z() - pOrigin.z(), 1.0F);
    ...
    public static Direction calculateFacing(int[] pFaceData) {
        Vector3f vector3f = new Vector3f(
                Float.intBitsToFloat(pFaceData[0]),
                Float.intBitsToFloat(pFaceData[1]),
                Float.intBitsToFloat(pFaceData[2]));
        Vector3f vector3f1 = new Vector3f(
                Float.intBitsToFloat(pFaceData[8]),
                Float.intBitsToFloat(pFaceData[9]),
                Float.intBitsToFloat(pFaceData[10]));
        Vector3f vector3f2 = new Vector3f(
                Float.intBitsToFloat(pFaceData[16]),
                Float.intBitsToFloat(pFaceData[17]),
                Float.intBitsToFloat(pFaceData[18]));
