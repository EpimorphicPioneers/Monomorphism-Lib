...
public class CustomWorkableMachineRenderer extends TieredHullMachineRenderer {
    ...
    public CustomWorkableMachineRenderer(int tier, ResourceLocation workableModel, Predicate<MetaMachine> isActive) {
        this(tier, workableModel, isActive, isActive);
    }

    public CustomWorkableMachineRenderer(int tier, ResourceLocation workableModel, Predicate<MetaMachine> isWorkingEnabled, Predicate<MetaMachine> isActive) {
    ...
    public void renderMachine(List<BakedQuad> quads, MachineDefinition definition, @Nullable MetaMachine machine, Direction frontFacing, @Nullable Direction side, RandomSource rand, Direction modelFacing, ModelState modelState) {
        ...
        if (machine != null) {
            quads.addAll(overlayModel.bakeQuads(side, frontFacing, isActive.test(machine), isWorkingEnabled.test(machine)));
        } else {
