...
public class CustomPartRenderer extends WorkableCasingMachineRenderer implements IControllerRenderer {
    ...
    public CustomPartRenderer(ResourceLocation baseCasing, ResourceLocation workableModel, Function<IMultiPart, ResourceLocation> locationGetter) {
    ...
    public void renderPartModel(List<BakedQuad> list, IMultiController iMultiController, IMultiPart iMultiPart, Direction direction, @Nullable Direction direction1, RandomSource randomSource, Direction direction2, ModelState modelState) {
        if (direction1 != null && direction2 != null) {
            list.add(FaceQuad.bakeFace(direction2, ModelFactory.getBlockSprite(locationGetter.apply(iMultiPart)), modelState));
        }
    }
}
