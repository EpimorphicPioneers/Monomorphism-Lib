...
public class CapabilityHandler {
    ...
    private CapabilityHandler() {
        ...
        Monomorphism.instance.registerEventHandler(this);
    }

    @SuppressWarnings("unchecked")
    public <T extends ICapabilityProvider, C> void registerCapability(
            ICapabilityImplementation<T, C> implementation) {
        this.capabilityImplementations.add(
                (ICapabilityImplementation<ICapabilityProvider, ?>) implementation);
    }

    @SubscribeEvent
    @SuppressWarnings({"unchecked", "unused"})
    // We must not define a generic type, or this method will not pass the AMSEventHandler generics
    // filter test
    ...
    @SuppressWarnings("unchecked")
    private <T extends ICapabilityProvider> void addCapabilitiesParametric(
            AttachCapabilitiesEvent<T> event) {
        ...
        this.capabilityImplementations.stream()
                ...
                .forEach(
                        impl -> event.addCapability(impl.getCapabilityKey(), impl.createProvider(carrier)));
    }
    ...
    public void registerCapabilities(RegisterCapabilitiesEvent event) {
        this.capabilityImplementations.stream()
                .filter(impl -> impl instanceof IMOCapabilityImplementation)
                .map(impl -> (IMOCapabilityImplementation<?, ?>) impl)
    ...
    public void onPlayerClone(PlayerEvent.Clone event) {
        ...
        if (oldPlayer != null && newPlayer != null && !oldPlayer.level().isClientSide()) {
            this.capabilityImplementations.stream()
                    .filter(impl -> Player.class.isAssignableFrom(impl.getCarrierClass()))
                    .filter(impl -> impl instanceof IMOCapabilityImplementation)
                    .map(impl -> (IMOCapabilityImplementation<?, ?>) impl)
                    .forEach(impl -> oldPlayer
                            .getCapability(impl.getCapability(), null)
                            .ifPresent(oldProps -> newPlayer
                                    .getCapability(impl.getCapability(), null)
                                    .ifPresent(newProps -> impl.copyData(oldProps, newProps))));
        }
    }
}
