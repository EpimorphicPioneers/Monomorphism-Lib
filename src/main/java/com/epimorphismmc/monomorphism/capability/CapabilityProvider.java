...
public final class CapabilityProvider<V> implements ICapabilitySerializable<CompoundTag> {
    ...
    public CapabilityProvider(Supplier<Capability<V>> capability, IMOCapabilityImplementation.Serializer<V> serializer, V value) {
    ...
    protected Optional<Capability<V>> checkCapability() {
        ...
        if(capability == null) {
            Monomorphism.instance.getLogger().error(
                    "[SEVERE] Capability implementation is requested before injection, report this to the mod author");
            new RuntimeException("[SEVERE] Encountered null capability for: " + this.getCapabilityValue().getClass().getName()).printStackTrace();
        }
